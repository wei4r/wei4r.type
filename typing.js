...
function newGame() {
  var ingame = document.getElementsByClassName('ingame');
  for (var i=0; i<ingame.length;i++)ingame[i].style.display='flex';  
  var outgame = document.getElementsByClassName('outgame');
  for (var i=0; i<outgame.length;i++)outgame[i].style.display='none';

  //refresh words
  document.getElementById('words').innerHTML = '';
  for (let i = 0; i < 30; i++)document.getElementById('words').innerHTML += formatWord();
  addClass(document.querySelector('.word-box'), 'current');
  addClass(document.querySelector('.letter'), 'current');
  document.getElementById('info').innerHTML = (gameTime / 1000) + '';
  while (document.querySelector('#game.over'))removeClass(document.getElementById('game'), 'over');
...
function getWpm() {
  ...
  return Math.round(correct_word_num / gameTime * 60000);
}
function gameOver() {
  ...
  for (var i=0; i<ingame.length;i++){
    ingame[i].style.display='none';
  }
  var outgame = document.getElementsByClassName('outgame');
  for (var i=0; i<outgame.length;i++){
    outgame[i].style.display='flex';
  }
...
function checkCorrect(thisLetter){
  var flag = thisLetter;
  while(flag.previousSibling){
    if(flag.classList.contains('incorrect')){
      return false;
    }
...
document.getElementById('game').addEventListener('keydown', ev => {
  ...
  if (isBackspace) {
    if (currentLetter && isFirstLetter) {
      ...
      removeClass(currentWord.previousSibling.children[0], 'incorrect');
      removeClass(currentWord.previousSibling.children[0], 'correct');
  ...
  // move lines / words
  if (isLastLetter&&currentWord.nextSibling.getBoundingClientRect().top > 430||currentWord.getBoundingClientRect().top > 430) {
    document.getElementById('cursor').style.display='none';
    const words = document.getElementById('words');
    const margin = parseInt(words.style.marginTop || '0px');
    words.style.marginTop = (margin - 95) + 'px';
    for (let i = 0; i < 10; i++)document.getElementById('words').innerHTML += formatWord();
  ...
  const nextWord = document.querySelector('.word-box.current').children[1];
  const cursor = document.getElementById('cursor');
  if(nextLetter.getBoundingClientRect().top<430)cursor.style.top = (nextLetter || nextWord).getBoundingClientRect().top + 5 + 'px';
  cursor.style.left = (nextLetter || nextWord).getBoundingClientRect()[nextLetter ? 'left' : 'right'] + 'px';
...
window.onload = function () {
  for (i = 0; i < 3; i++) {
    var button = document.getElementsByClassName("time")[i];
    button.onclick = function () {
      gameTime = this.innerHTML * 1000;
      clearInterval(window.timer);
      newGame();
    }
  }
