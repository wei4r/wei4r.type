...
function getResult() {
  ...
  console.log('incorrect: '+incorrect_word_num);
  for (var i = 0; i<correct_words.length; i++){
    correct_word_num += correct_words[i].innerText.length;
  }
  return [Math.round(correct_word_num / gameTime * 60000) , Math.round(100*(correct_word_num/(correct_word_num+incorrect_word_num)))];
}
function gameOver() {
  ...
  document.getElementById('wpm').innerHTML = `${wpm}<br>`;
